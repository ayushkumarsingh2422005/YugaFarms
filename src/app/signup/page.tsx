  ...
  return (
    <div className="min-h-[70vh] bg-[#fdf7f2] flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white/80 backdrop-blur rounded-xl shadow border border-[#2D2D2D]/10 p-6">
        <h1 className="text-2xl font-semibold text-[#4b2e19] text-center">Create account</h1>
        <p className="text-sm text-[#2D2D2D]/70 text-center mt-1">Join YugaFarms</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            <label className="block text-sm text-[#2D2D2D]/80 mb-1">Username</label>
            <input
              type="text"
              value={username}
        ...
        <p className="text-center text-sm text-[#2D2D2D]/70 mt-4">
          Already have an account? <Link href="/login" className="text-[#4b2e19] underline">Log in</Link>
        </p>
      </div>
    </div>
  );
